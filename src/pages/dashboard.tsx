...
import { TruckListPage as ShowCaseList } from 'pages/trucks';

function DashboardPage() {
  const { session } = useSession();

  const truckStat = useSWR<PaginatedInterface<TruckInterface>>(
    () => `/trucks`,
    () => getTrucks({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Trucks',
      result: truckStat?.data?.totalCount,
      isLoading: truckStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`trucks`} passHref legacyBehavior>
                  <Button
