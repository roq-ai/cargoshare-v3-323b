...
type ColumnType = ColumnDef<RouteInterface, unknown>;
...
export function RouteListPage(props: RouteListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRoutes({
        relations: ['truck', 'booking.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['origin.contains', 'destination.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RouteInterface>>(
    () => `/routes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RouteInterface) => {
    if (hasAccess('route', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('truck', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'truck',
          header: 'Truck',
          accessorKey: 'truck',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/trucks/view/${record.truck?.id}`}>
              {record.truck?.name}
            </Link>
          ),
        }
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="route" />
        </Flex>

        {hasAccess('route', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/routes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
