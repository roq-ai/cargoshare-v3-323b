...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBookings({
        relations: ['user', 'truck', 'route', 'transaction.count'],
  ...
  const columns: ColumnType[] = [
    { id: 'cargo_space_booked', header: 'cargo_space_booked', accessorKey: 'cargo_space_booked' },
    { id: 'booking_cost', header: 'booking_cost', accessorKey: 'booking_cost' },
    ...
    hasAccess('route', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'route',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/routes/view/${record.route?.id}`}>
              {record.route?.origin}
            </Link>
          ),
        }
